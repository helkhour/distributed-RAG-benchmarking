...
from config import MODEL_CONFIGS, VERBOSE, BATCH_SIZE
import logging
import time
...
class EmbeddingGenerator(nn.Module):
    ...
    def __init__(self, model_name, embedding_size, quantize=False):
        ...
        if self.device == "cpu":
            logger.warning("CUDA not available, falling back to CPU. Ensure CUDA is installed and configured.")
        logger.debug(f"Device selected: {self.device}")

        self.base_model = MODEL_CONFIGS[model_name].get("base_model", model_name)
        # logger.debug(f"Loading base model: {self.base_model}")
    ...
    def generate_embedding(self, texts):
        ...
        try:
            ...
            # logger.debug(f"Encoding completed, embedding shape: {embedding.shape}")
        ...
        if embedding.dim() == 1:
            ...
            # logger.debug("Unsqueezed embedding to add batch dimension")
        
        if self.projection:
            if embedding.dtype != self.projection.weight.dtype:
                embedding = embedding.to(dtype=self.projection.weight.dtype)
                # logger.debug(f"Converted embedding dtype to {self.projection.weight.dtype}")
            embedding = self.projection(embedding)
            # logger.debug("Applied projection layer")
        
        embeddings = embedding.cpu().tolist()
        # logger.debug(f"Converted embeddings to list, length={len(embeddings)}")
